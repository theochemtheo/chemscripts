...
for i in $(cat diabats.lst); do
	...
	if [ $DIABCHECK -ge "1" ]; then
		...
		if [ -f $NAME.diadipoles ]; then
			echo "Dipoles of diabatic states already extracted from $NAME.out"
		else
			# loop through just the diabatic states
			for ((i=1;i<=$NSTATES;i++)); do
				# Set up the search string for this state
				printf -v SEARCHSTR 'Excited-State Multipoles, State%3s' $i
				awk -v s="$SEARCHSTR" '$0 ~ s {getline;getline;getline;getline;getline;getline;printf "% 1.4f\n", $2}' $NAME.out >> $NAME.diadipoles
			done
		fi		
	else
